...
FlowInputNodeBase
{
    ...
    */
    function populateFlowNode( jsonData ) {
       ...
       else if( AppScripts.isDefined( jsonData.InputDescriptor ) ) 
            parseInputDescriptors( jsonData )
       else if( AppScripts.isDefined( jsonData.EventValues ) ) 
            parseEventValues( jsonData )   
       else if( AppScripts.isDefined( jsonData.UniformLayout ) )
    ...
    */
    ...
    function parseUniformLayout( jsonData )
    {
        ...
        for( let i in jsonData.UniformLayout ) {
            const ul = jsonData.UniformLayout[i]
            const ulType = ul.Type;
